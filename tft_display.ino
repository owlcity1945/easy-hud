...
void target_from_bno055(float &x, float &y) {
  ...
  const float alpha = 0.05f;                                 // 滤波因子（可以根据需要调整，通常在 0.05 到 0.2 之间）
...
void draw_target_box(TFT_eSprite &spr, float center_x, float center_y) {
  ...
  // 缓动惯性跟随，✅ 让俯仰梯跟随小球轨迹变化
  float slow_k = 0.05f;  //惯性系数，越小越飘逸	0.05 ~ 0.1
...
uint8_t rbuf[13];
uint8_t idx = 0;
void receive_from_stm32(TFT_eSprite &spr, int x, int y) {
  ...
  //     if (index >= 9) {
  //       // 收到完整帧
  //       memcpy(&pemh, &rx_buffer[1], sizeof(pemh));
  //       memcpy(&peml, &rx_buffer[5], sizeof(peml));
  //       index = 0;
  //     }
  //   }
  // }
  ...
  while (mySerial.available()) {
    ...
    if (idx == 0) {
      ...
      rbuf[idx++] = b;
      if (idx >= 13) {
        memcpy(&pemh, &rbuf[1], 4);
        memcpy(&peml, &rbuf[5], 4);
        memcpy(&p_xs, &rbuf[9], 4);
        idx = 0;
      }
    }
  }
  ...
  spr.drawString("pemh:", x, y);      // 显示 "pemh:"
  spr.drawFloat(pemh, 1, x + 30, y);  // 显示 pemh（保留 1 位小数）

  spr.drawString("peml:", x, y + 10);      // 显示 "peml:"
  spr.drawFloat(peml, 1, x + 30, y + 10);  // 显示 peml（保留 1 位小数）

  spr.drawString("p_xs:", x, y + 20);      // 显示 "peml:"
  spr.drawFloat(p_xs, 1, x + 30, y + 20);  // 显示 peml（保留 1 位小数）
}
// ESP32 中的 pemh/peml 值反馈给 STM32
void send_to_stm32() {

  uint8_t send_buffer[13];
  send_buffer[0] = 0xAA;  // 帧头
  memcpy(send_buffer + 1, &pemh, sizeof(pemh));
  memcpy(send_buffer + 5, &peml, sizeof(peml));
  memcpy(send_buffer + 9, &airpump_state, sizeof(airpump_state));
  mySerial.write(send_buffer, sizeof(send_buffer));
}
...
void words_display() {
  ...
  ladder_color_index = abs(encoderPos / 4) % 8;  //每4步更换一种颜色，限定颜色索引在 0~10 之间,避免负数导致越界
  ...
  receive_from_stm32(spr, 150, 10);                 //与stm32传输压力数据
  show_fps(spr);
